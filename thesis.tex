...
It only depends on $\NS$ if we include the anomalous dimensions $\eta_{\rm TT}$ and $\eta_{\sigma}$.
...
\frac{1}{6\pi}g_3^2\left(20-\frac{\NS}{4}+\frac{23}{60}\eta_S\right)
+\mathcal{O}(g_3^3).
\ee
Using
\be
...
\begin{figure}
  ...
  \caption{
    ...
    fixed point value for $g_3$, critical exponent $\theta$, and the anomalous dimensions $\eta_{\rm TT}$,
    $\eta_{\sigma}$ and $\eta_S$ as a function of the number of scalars $\NS$.
  }
...
\be
\beta_{\bar{G}}\Big|_{\rm scalar}=\frac{\bar{G}^2}{24 \pi}\NS (4-\eta_S).
\ee
...
+ \frac{\eta_{\sigma}}{24}
-  \frac{\NS}{24} (4 - \eta_{S})\right).
\eea

If we set $\eta_{\rm TT}= \eta_{\sigma}=0=\eta_S$, we obtain $\NS=45$ as the maximal
...
For the approximation where we set $\eta_S=0$, we obtain a bound at $\NS=12$,
