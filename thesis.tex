...
\begin{itemize}
  ...
  \item
    The beta function is finite. In particular, ultraviolet divergences
    that may appear on the right hand side of the renormalisation group
    equation cancel each other.
  \item
    The renormalisation group is non-perturbative, as it does not rely on the use of a small
    expansion parameter.
  \item
...
\begin{figure}
  \begin{center}
    \begin{tikzpicture}
      \begin{groupplot}[
        ...
        \addplot[blue,thick,samples=50]{ x / ( exp(x) - 1 )};
        ...
        \addplot[black,thick,dashed,samples=50,domain=0.1:3]{ 1/x };
        \addplot[blue,thick,samples=50]{ 1/( x + x / ( exp(x) - 1 ))};
        \addplot[green,thick,samples=100]{ 1/( x + x^2 / ( exp(x^2) - 1 ))};
        \addplot[red,domain=0:1,thick]{ 1 - 0.002 };
        \addplot[red,domain=1:3,thick]{ 1/x };
        \legend{
          \, $\frac{ 1 }{ y }$,
        }
  ...
  \caption{
    ...
    (right) their resulting propagators.
  }
  \label{fig:profiles}
\end{figure}
