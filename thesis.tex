...
In quantum gravity, quantum fluctuations of matter and geometry induce a scale dependence of the Newton coupling, just as quantum fluctuations of matter induce running matter couplings in the Standard Model. Therefore, a central object of interest in quantum gravity becomes the running Newton coupling and in particular its high-energy behavior. In practice, different ways of reading off the scale-dependence of the Newton coupling can be implemented.
...
of fields $\phi_i$ involved (notation from \cite{Meibohm:2016mkp}. These couplings can be viewed as ``avatars" of the Newton coupling.
In
...
fundamental coupling of the scalars to gravity. If diffeomorphism invariance is intact, these couplings are of course all equal to each other. Additionally, the scale-dependence of the Newton coupling can also be read off using the background-field formalism, where it is of course associated to the running prefactor of the curvature term in the action and will be referred to as $\bar{G}$. The main focus of this paper will be to compare the scale dependence of these couplings under the impact of quantum fluctuations of the metric and of $N_S$ scalar fields, and in various approximations.

The present functional renormalisation group approach builds on a
gauge-fixed approach to quantum gravity. The gauge fixing is implemented with the help of the background field formalism, such that background gauge dependence is respected. In contrast to non-Abelian gauge theories, the use of the background field method is unavoidable in setting up a Renormalisation Group flow in the geometric phase of quantum gravity, as the distinction of high-momentum and low-momentum modes, inherent to the coarse-graining procedure, requires the introduction of a background. 
The resulting effective
action $\Gamma[\bar g_{\mu\nu},h_{\mu\nu}]$ is not diffeomorphism-invariant but
satisfies non-trivial Slavnov-Taylor identities (STIs). Moreover, the
introduction of a background metric $\bar g_{\mu\nu}$ is inherent to the
...
\begin{align}\label{eq:linpara}
  g_{\mu \nu}=\bar g_{\mu \nu}+h_{\mu \nu}.
\end{align}
Additionally, we rescale the fluctuation field $h_{\mu\nu}$ by a dimensionfull constant of mass-dimension -1, such that $h_{\mu\nu}$ has a canonical dimension 1 for the bosonic fluctuation field $h_{\mu \nu}$. If we derive all pure-graviton and graviton-matter indices from a diffeomorphism invariant action containing an Einstein-Hilbert term with Newton coupling $\bar{G}_N$, then that dimensionfull constant should be $\sqrt{\bar{G}_N}$. In a second step, we then reabsorb the corresponding power of $\bar{G}_N$ in a the fluctuation field vertices. 
...
unique Newton coupling $G$ would be present. This property indeed
holds for the diffeomorphism-invariant effective action
\begin{align}
\Gamma[g_{\mu\nu}]= \Gamma[g_{\mu\nu},0]\,.
\end{align}
This already sets its vertices $\Gamma^{(n)}[\bar g_{\mu\nu}]$ apart from
$\delta^n/\delta h^n \Gamma[\bar g_{\mu\nu},h_{\mu\nu}]$. In renormalisable theories with
...
setting with mSTIs and mNIs we may hope for {\it effective
  universality} based on the underlying diffeomorphism invariance. For
...
version is called $G_{(3,0)}$. The second one is defined from the
graviton-two-scalar vertex as in \cite{Dona:2015tnf}, and is called
$G_{(1,2)}$. We use a symmetric momentum configuration. For the analytical
...
\end{eqnarray}
we insert the linear parameterisation Eq.~\eqref{eq:linpara} and 
...
In the following, all pure-graviton couplings will be set equal to $G_{(3,0)}$, i.e., $G_{n,0}= G_{(3,0)}$, and 
all graviton-matter couplings will be set equal to $G_{(1,2)}$, i.e., $G_{n,m}=G_{(1,2)}$ (for $m>0$). 
We then obtain
%
\begin{eqnarray}
\beta_{G_{(3,0)}}&=& \beta_{G_{(3,0)}}|_{\rm grav} + 2 G_{(3,0)} + 3G_{(3,0)}\, \eta_h \phantom{\frac{1}{1}}\nonumber\\
&{}& - G_{(3,0)}^{1/2} \,G_{(1,2)}^{3/2}\, N_S\frac{86+\eta_S}{1140 \pi}, \label{eq:betaG3}\\
%
\beta_{G_{(1,2)}}&=& \left(2 +\eta_h+ 2\eta_S\right)G_{(1,2)}  +  \frac{2\sqrt{G_{(1,2)}}}{3\pi} \Bigl(-G_{(1,2)}^{3/2} \frac{6-\eta_h}{(\mu+1)^2} \nonumber\\
&{}&+ \sqrt{G_{(3,0)}}G_{(1,2)} \frac{3(8-\eta_h)-16\lambda_3(6-\eta_h)}{6(\mu+1)^3} \Bigr)\label{eq:betag3},\\
%
...
Whereas the $N_S$ dependence in $\beta_{G_{(3,0)}}$ comes with $-\,\sqrt{G_{(3,0)}}G_{(1,2)}^{3/2}$, there is no $N_S$ dependence in $\beta_{G_{(1,2)}}$ (as there is no closed scalar loop that contributes). 
...
\begin{equation}
\eta_h = -\eta_h\Big|_{\rm grav} + G_{(1,2)} \frac{N_S}{24 \pi}.
\end{equation}
If we insert this into Eq.~\eqref{eq:betaG3} and \eqref{eq:betag3}, we obtain the contribution $\sim N_S$ as follows:
\begin{eqnarray}
\beta_{G_{(3,0)}}\Big|_{N_S}&=& 3 G_{(3,0)} G_{(1,2)} \frac{N_S}{24 \pi}  - G_{(3,0)}^{1/2} \,G_{(1,2)}^{3/2}\, N_S\frac{86+\eta_S}{1140 \pi}\nonumber\\
&=& g^2  N_S \frac{113}{2280 \pi} \approx 0.016\, g^2 N_S,\nonumber\\
\beta_{G_{(1,2)}} \Big|_{N_S}&=& G_{(1,2)}^2 \frac{N_S}{24 \pi}\nonumber\\
...
Further, we observe that the same sign of the coefficient as in $\beta_{G_{(1,2)}}$ has been obtained within the exponential parameterization and the so-called physical gauge, see Eq.~59 in \cite{Dona:2015tnf}.
...
\dot G_{(3,0)} &=& 2G_{(3,0)} + 3 \eta_h(k^2) G_{(3,0)} -\frac{24}{19}\left(\eta_h(k^2)
  ...
  \frac{(32\pi)^2\sqrt{G_{(3,0)}}}{k}\left(\text{Flow}^{(hhh)}_{{\rm TT},G_{(3,0)}}(k^2)-
    ...
    \dot G_{(1,2)} &=& 2 G_{(1,2)} + \eta_h(k^2)G_{(1,2)} +2 \eta_S(k^2)G_{(1,2)}\\
    &{}&+ \frac{32 \pi\, \sqrt{G_{(1,2)}}}{k}\frac{8}{3}\text{Flow}^{(h\varphi\varphi)}_{{\rm TT},G_{(1,2)}}(k^2).\nonumber
\end{eqnarray}
%
Here, the notation ${\rm TT}, G_{(3,0)}/G_{(1,2)}$ indicates a contraction of the right-hand-side of the Wetterich equation with the projector onto the corresponding coupling, as in \cite{Meibohm:2015twa} and \cite{Dona:2013qba}.

We then obtain a qualitatively similar behaviour with $N_S$ as in the analytical case, cf.~\autoref{fig:NewtoncouplingFPs}. 
...
Moreover, the behaviour is in agreement with that of $G_{(1,2)}$ in \cite{Dona:2015tnf}, which employed a different choice of parameterisation, gauge, and projection. 
This robustness with respect to (unphysical) variations of the scheme is encouraging.

%
\begin{figure}[b!]
\includegraphics[width=\linewidth]{NS_findiff_all_hss_all_g_new.pdf}
...
As $\mu$ stays approximately constant as $N_S$ increases, while $G_{(3,0)}$ increases, $g_{\rm eff,\,1/2}$ increase as well, cf.~\autoref{fig:effectiveG}.

%
\begin{figure}[!t]
\includegraphics[width=\linewidth]{NS_findiff_all_hss_eff_g.pdf}
\caption{\label{fig:effectiveG} We plot the effective gravitational
  strength $G_{(3,0)}/(1+\mu)$ and $G_{(3,0)}/(1+\mu)^2$. The latter appears
...
%
\begin{eqnarray}
\beta_{G_{(3,0)}} &=& 2 G_{(3,0)} + 3 G_{(3,0)} \eta_h - \sqrt{G_{(3,0)}}G_{(1,2)}^{3/2}N_S\, \frac{86}{1140 \pi},\\
\beta_{G_{(1,2)}}&=& 2 G_{(1,2)} + G_{(1,2)} \eta_h,\\
\eta_h &=& G_{(1,2)} N_S \frac{1}{24 \pi},\\
\eta_S&=&0.
\end{eqnarray}
%
...
\theta_1=2, \theta_2 \rightarrow - i\, \infty .
\end{eqnarray}
...
general split Ward identities have also been discussed in
% \cite
...
\begin{align}\label{eq:Fg}
	F_{\text{\tiny RG}}(x)= \int_{x_0}^x \mathrm dx' \; \0{\partial R_k(x')}{\partial x'} \, G_k(x', \bar R = 0)\,,
\end{align}
...
\begin{align}
 ...
 &= \0{\delta}{\delta \bar g_{\mu\nu}} \int_{0}^{\bar\Delta} d x' \0{\partial R_k(x')}{\partial x'} G_k(x', \bar R = 0) \notag \\
 &= \0{\delta \bar \Delta}{ \delta \bar g_{\mu\nu}} \0{\partial R_k(\bar \Delta)}{\partial \bar \Delta} G_k(\bar \Delta, \bar R = 0) \notag \\ 
...
\begin{align}\nonumber 
  \012 \Tr&\left[ \0{1}{\sqrt{\bar g}}\0{\delta \sqrt{\bar g} \, R_k(\bar \Delta)}{\delta \bar g_{\mu\nu}}\,G_k(\bar \Delta, \bar R )\right]\\[2ex]  
  =&\frac{1}{2} \, \Tr\, \0{\delta }{\delta \bar g_{\mu\nu}} F_{\text{\tiny{RG}}}(\bar\Delta) 
  +\frac{1}{2} \Tr \, \0{1}{\sqrt{\bar g}}\0{\delta \sqrt{\bar g}}{\delta \bar g_{\mu\nu}}\, R_k(\bar \Delta) G_k (\bar \Delta, \bar R ) \,.
\label{eq:g-Rkg}
\end{align}
...
These are on the one hand necessary for comparison and on the other hand we can express the level one couplings as background plus a Nielsen identity
...
The results for gravity part of the background flows agree with e.g.~\cite{Gies:2015tca}
and the scalar part agrees with e.g.~\cite{Dona:2013qba}.
The details of the computation are given in appendix \ref{app:traces}.

The flow of the background couplings is given by
\begin{align}
 \partial_t \bar G &= (2 + \eta_N) \bar G \,,\\
 \partial_t \bar \Lambda &= - 4 \bar \Lambda + \0{\bar \Lambda}{\bar G} \partial_t \bar G + 8 \pi \bar G \Flow_{\bar \Gamma}\bigg|_{\sqrt{\bar g} \text{-terms}} \,,\\
...
\begin{align}
 ...
 - \partial_t \left( \012 \Tr \01{\sqrt{\bar g}} \0{\delta \sqrt{\bar g} R_k }{ \delta {\bar g}_{\mu\nu} } G_k \right)
 \,.
\end{align}
...
The flow equations for the level one couplings can now be expressed by flow of the background couplings plus the improvement from the Nielsen identity, to wit
\begin{align}
 \partial_t \bar G_1 &= (2 + \eta_{N,1}) \bar G_1 \,, \\
 \partial_t \bar \Lambda_1 &= - 4 \bar \Lambda_1 + \0{\bar \Lambda_1}{\bar G_1} \partial_t \bar G_1 + 8 \pi \bar G_1 \left(  \Flow_{\bar \Gamma} - 8 I \right) \bigg|_{\sqrt{\bar g} \text{-terms}} \,, \\
...
\vspace{.5cm}
\noindent \textbf{Acknowledgments} We thank N.~Christiansen,
S.~Lippoldt and R.~Percacci for discussions. MR
acknowledges funding from IMPRS-PTFS. This work is supported by EMMI
and by ERC-AdG-290623.
...
\begin{align}
  \bar R_{\mu\nu} = \01d \bar R\, \bar g_{\mu\nu} \,,
\end{align}
this gives 
\begin{align}
  \frac{\delta \bar R}{\delta \bar g_{\mu\nu}} =  - \01d \bar R\, \bar g^{\mu\nu} + \, \bar g^{\alpha\beta} \frac{\delta \bar R_{\alpha\beta}}{\delta \bar g_{\mu\nu}} \,.
\end{align}
We further know
\begin{align}
\frac{\delta \bar R_{\alpha\beta}}{\delta \bar g_{\mu\nu}} = \bar \Delta_\rho \0{\delta \Gamma^\rho_{\alpha\beta}}{\delta \bar g_{\mu\nu}} 
				  - \bar \Delta_\alpha \0{\delta \Gamma^\rho_{\rho\beta}}{\delta  \bar g_{\mu\nu}} \,.
\end{align}
These terms are total derivatives and will not contribute.
Note that this doesn't hold anymore when the derivative of $\bar R^2$ is evaluated, but we don't need the $\bar R^2$ derivative anyway.
This gives
\begin{align}
  \frac{\delta \sqrt{\bar g}}{\delta \bar g_{\mu\nu}} &= \012 \sqrt{\bar g}\, \bar g^{\mu\nu} \,,\\
...
\section{Nielsen Identities and Yang-Mills}
