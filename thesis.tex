...
\newcommand\etaSfluc{ \eta_{\scriptscriptstyle{\mathrm{S}},1} }
...
\newcommand\Gback{ \overbar{G} } % background Newton coupling
\newcommand\Lback{ \overbar{\Lambda} } % background cosmological constant
...
system coupled to a number of scalar fields. In doing so, we find a number
of scaling solutions in a fully functional truncation using the derivative
expansion in the background field approximation with a $d$-dimensional sphere
as the background.
...
linear parametrisation of the metric.
-- similar behaviour (robustness)
-- always find fixed points
...
To correct this behaviour we supplement the background field equations on
a sphere with the modified split Ward identities.
-- results?
...
\label{sec:resultsPureGravityCH4}
...
in Ch.~\ref{ch:tim}, one may thus ask if you can uncover some notion of `effective universality'
...
\begin{align}\label{eq:linpara}
  g_{\mu \nu}=\bar g_{\mu \nu}+h_{\mu \nu}.
\end{align}
for the metric fluctuations and we will not York--decomposed the respective
spin modes.
...
\begin{align}
  ...
  + \frac{ 2 \, \sqrt{ \vphantom{G^2} G_{(1,2)}} }{ 3\pi }
  + \Gback^2 \, \frac{\NS}{24\pi}(4-\etaS)
  \nonumber \\[3pt]
  &+
  \left(
...
and the $\NS$ dependence in $\beta_{\Gback}$ is proportional to $+\Gback^2$.
...
\begin{figure}[t]
  ...
  \caption{
    ...
    constants.
    % the three-graviton coupling (red),
    % the graviton-scalar-coupling (blue) and the background coupling
    % in two different gauges (orange and cyan).
...
To obtain numerical results, we will use a bi-local projection for the Newton couplings,
and include the anomalous dimension projected at scale $k$.
...
We then obtain a qualitatively very similar behaviour as in the analytical case,
\cf \autoref{fig:NewtoncouplingFPs}.
In particular, all three Newton couplings are driven towards larger
fixed point values when $\NS$ is increased.
Remarkably, the quantitative deviations between the three avatars are not particularly large.

Moreover, the behaviour is in agreement with that of $g_3$ in the last chapter,
where we employed a different choice of parametrisation, gauge, and projection.
The robustness of the results with respect to these unphysical variations of the
scheme is very encouraging.

When the fluctuation couplings enter the graviton anomalous dimension,
...
\section{Background field approximation and Ward identities}
...
% The full cutoff operator $R_k$ is a matrix in field space,
% \begin{align}
%   R_k =
%   \begin{pmatrix*}
%     R_h & 0    & 0   & 0 \\
...
%   \,.
% \end{align}
% and the functional traces above include a symplectic metric in the ghost subsector.
...
\begin{align}
 ...
 % = \frac{\delta \overbar \Delta}{ \delta \bar g_{\mu\nu}} \frac{\partial R_k(\overbar \Delta)}{\partial \overbar \Delta} G_k(\overbar \Delta, \bR = 0) \notag \\
...
In this section we have a look at the flow equations of the background couplings.
...
by setting $\lambda=\Lback$, $\eta_h=\etaS$ and $\eta_c=0$.


\section{Fluctuation couplings via Ward identities}

\begin{figure}[t]
  \begin{center}
    \input{figures/comparison-couplings.tex}
  \end{center}
  \caption{
    Comparison of three different definitions of gravitational couplings and
    cosmological constants:
    pure background system (red dashed), background system improved by msWI (blue),
    and fully fluctuating system (cyan).
    (Left) Background gravitational couplings using different avatars of the
    remaining couplings on the right hand side of the flow equation.
    (Right) Highest dynamical coupling of the three approximation schemes.
  }
  \label{fig:comparison-couplings}
\end{figure}

In this section we want to compute the running of the level one fluctuation couplings
using the modified split Ward identity together with the background flow equations of the last section.
As derived in Sec.~\ref{sec:Nielsen-id}, we want to make the following approximation
\begin{align}
  \frac{\delta}{\delta h_{\mu\nu}} \partial_t \Gamma_k
  \approx \frac{\delta}{\delta {\bar g}_{\mu\nu}} \partial_t \Gamma_k
  - \partial_t
  \left(
    \frac 12 \, \Tr \frac 1{\sqrt{\bar g}} \frac{\delta \sqrt{\bar g} R_k }{ \delta {\bar g}_{\mu\nu} } G_k
  \right) .
\end{align}
Consequently, in order to extract the running of the fluctuation couplings from the one of the
background couplings,
we need to compute the flow of the following quantity:
\begin{align}
  I \equiv&\, \frac 12 \, \Tr
  \left[
  ...
  \label{eq:def-I}
\end{align}
Here, $\FRG(\overbar \Delta)$ is the functional introduced in \eqref{eq:Fg}.
All functional traces appearing in the last equation can be evaluated using heat kernel techniques.
Details of the computation, as well as intermediate results may be found in App.~\ref{app:msWItraces}.
There, we find the following result for $I$:
\begin{align}
  \notag
  ...
  \Bigg\rbrace
    ...
    &+ \mathcal O (\bR^3) \,.
\end{align}
The flow equations for the level one fluctuation couplings can now be expressed
through the flow of the background couplings with the addition of the improvement
coming from the modified split Ward identity. This yields
\begin{align}
  \vphantom{ \frac{\Lback}{\Gback} }
  \partial_t \Gback_1
  &= \big( 2 + \etaSfluc \big) \, \Gback_1 \,, \\
  \partial_t \Lback_1
  &= - 4 \, \Lback_1 + \frac{\Lback_1}{\Gback_1} \, \partial_t \Gback_1
  + 8 \pi \Gback_1 \, \big(  \Flow_{\overbar \Gamma} - 8 I \big) \Big|_{\mathcal O(\bR^0)} \,, \\
  \vphantom{ \frac{\Lback}{\Gback} }
  \etaSfluc
  &= 16 \pi \Gback_1 \, \big( \Flow_{\overbar \Gamma} - 8 I  \big) \Big|_{\mathcal O(\bR^1)} \,.
\end{align}
The quantities in $I$ and $\Flow_{\overbar \Gamma}$ should be taken from the fluctuation two-point function.
...
For this we set $\lambda=\Lback_1$, $\eta_h=\etaSfluc$ and $\eta_c=0$.

\begin{figure}[t]
  \begin{center}
    \input{figures/all-from-fluc-and-mu.tex}
  \end{center}
  \caption{
    Comparison of different avatars of the gravitational couplings (left)
    and the cosmological constants (right).
    (Above) All couplings were evaluated with the input of the full fluctuation
    system on the right hand side of the flow equation.
    (Below) We combined the flow equation for the graviton mass parameter $\mu$
    with different flow equations for the Newton's coupling.
  }
  \label{fig:All-from-fluc}
\end{figure}

Whilst the gravitational couplings behave qualitatively similar,
the level-$n$ 'cosmological constants' display significant differences in their behaviour
\cf Fig.~\ref{fig:All-from-fluc}.
This emphasises the necessity to include the graviton mass parameter in a fluctuation field
computation.
...
% This result is surprising as the beta function is a universal quantity.
...
% ($\tr \, \overbar F^2$ in the Yang-Mills case)
...
% not expect qualitative differences in the UV.
% Still we remark that in Yang-Mills theory the (genuine) background anomalous dimension
% $\eta_{\overbar A}=-\partial_t \log Z_{\overbar A}=2 \beta_g$,
% and the fluctuating anomalous dimension
% $\eta_{\overbar a}=-\partial_t \log Z_{\overbar a}$
% differ by approximately a factor two (in the Landau gauge).
% Moreover, while the latter is gauge-dependent, the former is not.
% This difference is at the heart of the background approximation,
...
% but we don't need the $\bR^2$ derivative anyway.
We thus have in $d=4$
\begin{align}
