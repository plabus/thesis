...
In this section we will introduce the functional renormalisation group as
a tool to solve a quantum field theory. In particular, we will see how to
formalise Wilson's idea of a functional renormalisation group for the action
of a quantum field theory by introducing smooth infrared cutoff operators
which will depend on the energy scale. We will then introduce a scale-dependent
...
\begin{align}
  ...
  \big\langle  \varphi(x_1) \, \varphi(x_2) \, \dots \, \varphi(x_n) \big\rangle \,.
\end{align}
These are functional expectation values of products of $n$ field
...
\begin{align}
  ...
  \varphi(x_1) \, \varphi(x_2) \, \dots \, \varphi(x_n) \, e^{-S \lbrack \Phi \rbrack } \,,
\end{align}
where $Z$ is a normalisation constant, known as the partition function,
$\Phi$ represent the field content of the theory and $e^{-S \lbrack \Phi \rbrack}$
...
by means of the generating functional $W \lbrack \Phi \rbrack$, which is defined
via the relation
\begin{align}
  e^{ -W \lbrack \Phi \rbrack }
  = \int \mathcal D \Phi \;
  e^{-S \lbrack \Phi \rbrack + \int J \cdot \Phi } \,,
\end{align}
and taking derivatives with respect to the source term $J$
\begin{align}
  C_n(x_1, \, x_2,\, \dots, \, x_n) =
  \frac{ \delta W }{ \delta J(x_1) \, \delta J(x_2) \, \dots \, \delta J(x_n) } \bigg|_{J=0} \;,
\end{align}
which are evaluated at $J=0$. Solving a quantum field theory can
...
\end{align}
The generating functional may then be recovered by integrating out
the remaining modes
\begin{align}
  e^{ - W \lbrack \Phi \rbrack }
...
Our aim for this section is it to introduce a concept of a scale dependent
...
alter only the propagation of any mode, and not the interaction structure.
This is why we choose to introduce a bilinear term in the fields,
which in momentum space simply reads
\begin{align}
  \Delta S_k [\phi] = \frac 12 \int \frac{ \mathrm d^dq }{ (2\pi)^d } \;
  \varphi(-q) \, R_k(q^2) \, \varphi(q) \,.
\end{align}
Although we will outline the case of a scalar field here first, it will be
easy to generalise this procedure to other types of fields later on.
One should think of the regulator or cutoff operator $R_k(q^2)$ as a scale-dependent
...
\end{align}
This shows clearly, that we indeed introduced an infrared cutoff.
...
\begin{figure}
  \begin{center}
    \begin{enumerate}
      ...
      \item
        For any $k$ fixed, $R_k(q^2)$ is monotonically decreasing.
      \item
        For any $q^2$ fixed, $R_k(q^2)$ is monotonically increasing.
      \item
        For any $q^2 > k^2$, $R_k(q^2) \rightarrow 0$ sufficiently fast.
      \item
        $\lim_{k \rightarrow 0} R_k(q^2) = 0$.
      \item
        $R_k(0) = k^2$.
    \end{enumerate}
  \end{center}
  \caption{Formal requirements on a (standard) cutoff operator.}
  ...
  \begin{center}
    \begin{tikzpicture}
      \begin{groupplot}[
        ...
        \addplot[blue,thick,samples=100]{ x / ( exp(x) - 1 )};
        \addplot[green,thick,samples=100]{ x^2 / ( exp(x^2) - 1 )};
        \addplot[red,thick,domain=0:1,thick]{ 1 -x };
        \addplot[red,thick,domain=1:3,thick]{ 0.002 };
        \legend{
          \, $\frac{ y }{ e^y - 1}$,
          \, $\frac{ y^2 }{ e^{y^2} - 1}$,
        ...
        \addplot[black,thick,dashed,samples=50,domain=0.1:6]{ 1/x };
        \addplot[blue,thick,samples=100]{ 1/( x + x / ( exp(x) - 1 ))};
        \addplot[green,thick,samples=100]{ 1/( x + x^2 / ( exp(x^2) - 1 ))};
        \addplot[red,domain=0:1,thick]{ 1 - 0.002 };
        \addplot[red,domain=1:6,thick]{ 1/x };
        \legend{
          $\frac{ 1 }{ y }$,
        }
...
\end{align}
It will turn out to be more convenient
to pass to the
functional Legendre transform
of $W_k[J]$, called the effective
average action, which is defined by
\begin{align}
...
\end{align}
The first term can be re-expressed by observing
that
\begin{align}
...
\begin{align}
  ...
  \right\rangle .
\end{align}
% where $R^*$ is the cutoff operator in real space
Using the fact that the cutoff operator $R_k$ is independent
of the field $\tilde \varphi$ we can write the renormalisation
group time derivative of the effective average action as
\begin{align}
  \frac{ \mathrm d \Gamma_k }{ \mathrm dt }
  = \frac 12 \, \Tr
  \left[
...
over space-time and momentum. To make further progress, we
...
also called Wetterich equation, as follows
\begin{align}
  % \boxed{
    \frac{ \mathrm d \Gamma_k }{ \mathrm dt }
    = \frac 12 \; \Tr
