...
global and local (gauge) symmetries, one has to search for solutions
...
\end{align}
which is an infinite sum over a countable set $\mathcal I$,
where $\mathcal O_i [\Phi]$ represent all operators which can be build
with the given fields $\Phi$ and covariant derivatives in such a way
...
\footnote{%
  ...
  case of having a single scalar as field content in the theory.
}
\begin{align}
  \Gamma_k [\varphi] =
...
insertion $\partial_t R_k$ with the symbol ..., we can iconographically express the
...
\begin{align}
\end{align}
We will use the derivative expansion later on in chapters 4 and 5, and
