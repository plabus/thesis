...
\footnote{%
  \texit{A priori}, this has nothing to do with diffeomorphism invariance of
...
\begin{align}
  ...
  \sigma'  &=
  % \sqrt{-\bnabla^2}\sqrt{-\bnabla^2-\frac{\bR}{d-1} \, } \, \sigma \,.
  \sqrt{
    \left( -\bnabla^2 \right) ^2 + \frac{\bR}{d-1} \, \bnabla^2 \,
  } \, \sigma \,.
  \label{redef}
\end{align}
...
[F should be 1 here.]
Here we use the standard background gauge fixing term
\be
S_{GF}=\frac{F(\bphi)}{2\alpha}\int dx\sqrt{\bg}\,\bg^{\mu\nu}\chi_\mu\chi_\nu
\ee
with
\be
\chi_\mu=\bnabla_\rho h^\rho{}_\mu-\frac{\beta+1}{d}\bnabla_\mu(\tr h)\ .
\ee
Using the York decomposition, we find
\be
S_{GF}=\frac{F(\bphi)}{2\alpha}\int dx\sqrt{\bg}
\left[\xi'_\mu\left(-\bnabla^2-\frac{\bR}{d}\right)\xi^{\prime\mu}
+
\left(\frac{d-1}{d}\sqrt{-\bnabla^2-\frac{\bR}{d-1}}\sigma'+2\beta\sqrt{-\bnabla^2}\omega\right)^2\right]
\ee

\subsection{Transformation properties}

The transformation of the metric under an infinitesimal diffeomorphism $\epsilon$
...
As usual, we have to define transformations of $\bg$ and $h$ which
used in (\ref{decomp}) yield (\ref{transfg}).
The simplest one is the background transformation.
If we treat $\bg$ and $h$ as tensors under $\delta_\epsilon$, i.e.
\be
...
\ee
and (\ref{transfg}) follows.

By definition, the ``quantum'' gauge transformation of $h$ is such as to reproduce (\ref{transfg})
when $\bg$ is held fixed:
\be
...
+O(\epsilon h^2)\ .
\ee
The first three terms coincide with the quantum transformation when one uses
the linear background decomposition.

\subsection{Unimodular gauge}

paper Percacci, Vacca p. 9f + ghosts


\section{Variations of Christoffel symbols and curvature invariants}

The Riemann and Ricci tensors in our conventions are given by
\begin{align}
...
\begin{align}
  Q = Q^{\scriptscriptstyle(0)} + Q^{\scriptscriptstyle(1)} +
  Q^{\scriptscriptstyle(2)} + Q^{\scriptscriptstyle(3)} + \ldots \,,
\end{align}
...
+\frac{1}{2}(\tr h)\bnabla_\mu\bnabla_\nu h^{\mu\nu}
-\frac{1}{4}(\tr h)\bnabla^2 (\tr h)
\nonumber\\
&&
+\frac{1}{2}\bR_{\mu\rho\nu\sigma}h^{\mu\nu}h^{\rho\sigma}
-\frac{1}{2}\bR_{\mu\nu}h^{\mu\nu}(\tr h)
+\frac{1}{8}\bR(\tr h)^2\Bigr]
\eea

YORK: should use h etc here.

Collecting all terms we can rewrite the quadratic action in terms
of the independent fields $\tth$, $\xi$, $\sigma$, $\omega$ and $\varphi$:
\bea
...
\nonumber\\
&&
-\frac{(d-1)(d-2)}{d}\omega\sqrt{(-\bnabla^2)\left(-\bnabla^2-\frac{\bR}{d-1}\right)}\sigma'
-(d-1)(d-2)\omega\left(-\bnabla^2+\frac{(d-2)\bR}{2(d-1)}\right)\omega\Biggr)
\nonumber\\
&&
+F'(\bphi)\frac{d-1}{d}\varphi\left(
\sqrt{(-\bnabla^2)\left(-\bnabla^2-\frac{\bR}{d-1}\right)}\sigma'
-2d\left(-\bnabla^2+\frac{(d-2)\bR}{2(d-1)}\right)\omega
\right)
\nonumber\\
&&
+\frac{1}{2}\varphi(-\bnabla^2+V''(\bphi)-F''(\bphi)\bR)\varphi
+V'(\bphi)d\omega\varphi
+\frac{1}{2}V(\bphi)d^2\omega^2\Biggr]
\eea
We note that the kinetic operator of the $\omega$ field is the
conformal scalar operator.

\subsection{Conventional gauge fixing}

Adding the gauge fixing terms to the second variation of the action one obtains
\bea
\label{hesstot}
&&\int dx\sqrt{\bg}\Biggl[
F(\bphi)\Biggl(
\frac{1}{4}\tth_{\mu\nu}\left(-\bnabla^2+\frac{2\bR}{d(d-1)}\right)\tth^{\mu\nu}
+\frac{1}{2\alpha}\xi'_\mu\left(-\bnabla^2-\frac{\bR}{d}\right)\xi^{\prime\mu}
\nonumber\\
&&
+\frac{(d-1)(2(d-1)-(d-2)\alpha)}{4d^2\alpha}
\sigma'\left(-\bnabla^2-\frac{2\bR}{2(d-1)-(d-2)\alpha}\right)\sigma'
\nonumber\\
&&
-\frac{(d-1)((d-2)\alpha-2\beta)}{d\alpha}\omega\sqrt{(-\bnabla^2)\left(-\bnabla^2-\frac{\bR}{d-1}\right)}\sigma'
\nonumber\\
&&
-\frac{(d-1)(d-2)\alpha-2\beta^2}{\alpha}
\omega\left(-\bnabla^2+\frac{\alpha(d-2)^2\bR}{2((d-1)(d-2)\alpha-2\beta^2)}\right)\omega\Biggr)
\nonumber\\
&&
+F'(\bphi)\frac{d-1}{d}\varphi\left(
\sqrt{(-\bnabla^2)\left(-\bnabla^2-\frac{\bR}{d-1}\right)}\sigma'
...
check stuff from app. C here too.
...
\chapter{Vertices in the Einstein-Hilbert action}
\label{app:EH}

\section{Vertices}

Note that in $\delta^3 R_{\mu \nu}$ the first 2 lines do not contribute to $\delta^3 \int d^dx\, \sqrt{g}\, R$,
as they are total derivatives.
On a flat background $\mathbb R^4$ we find
\bea
...
\Bigr)
\eea
When $h=0$ is imposed (either on the level of the configuration space,
as in unimodular gravity, or as a choice of gauge), only the last four terms remain,
\ie the vertex simplifies very considerably.
