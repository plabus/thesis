...
Starting from an Einstein-Hilbert term
(augmented by a gauge fixing and ghost action) and a kinetic term for the scalar,
\begin{align}
  S &= S_{\rm EH} + S_{\rm gf} + S_{\rm gh} + S_{\rm scalar}\nonumber\\
    &= -\frac{1}{16 \pi \GNewton}
  \int \mathrm d^4x \, \sqrt{g} \;
  \left( R - 2 \Lambda \right) + S_{\rm gf} + S_{\rm gh} \nonumber\\
  &+\frac{1}{2}\sum_{i=1}^{\NS} \int \mathrm d^4x \, \sqrt{g} \;
  g^{\mu \nu} \partial_{\mu}\varphi^i\partial_{\nu}\varphi^i\,,
\end{align}
we insert the linear parametrisation Eq.~\eqref{eq:linpara} and
...
Accordingly, we introduce separate dimensionful couplings, denoted by $\bar{G}_{(n_h, n_{\varphi})}$ in front of these vertices.
Similarly, we distinguish the different incarnations of the cosmological constant
by introducing the dimensionful graviton mass parameter $\bar{\mu}$ associated to $- 2 \Lambda$ and
...
In the following,
all pure-graviton couplings will be set equal to $G_{(3,0)}$, i.e., $G_{n,0}= G_{(3,0)}$,
and all graviton-matter couplings will be set equal to $G_{(1,2)}$, i.e., $G_{n,m}=G_{(1,2)}$ (for $m>0$).
We then obtain
\begin{align}
  \beta_{G_{(3,0)}} &= \beta_{G_{(3,0)}}|_{\rm grav} + 2 G_{(3,0)} +
  3G_{(3,0)}\, \eta_h \phantom{\frac{1}{1}}\nonumber\\
                    & - G_{(3,0)}^{1/2} \,G_{(1,2)}^{3/2}\, \NS\frac{86+\eta_S}{1140 \pi}, \label{eq:betaG3}\\
  \beta_{G_{(1,2)}} &= \left(2 +\eta_h+ 2\eta_S\right)G_{(1,2)}
   + \frac{2\sqrt{G_{(1,2)}}}{3\pi} \Bigl(-G_{(1,2)}^{3/2} \frac{6-\eta_h}{(\mu+1)^2} \nonumber\\
  &+ \sqrt{G_{(3,0)}}G_{(1,2)} \frac{3(8-\eta_h)-16\lambda_3(6-\eta_h)}{6(\mu+1)^3} \Bigr)\label{eq:betag3},\\
  &+ \bar{G}^2\, \NS\, \frac{1}{24\pi}(4-\eta_S).
  \end{align}
The shown background results are the same as in \cite{Dona:2013qba} since the contribution from
the scalar loop is independent of the gauge.
Our focus is on the $\NS$-dependent part.
If we neglect for now that $\eta_h$ can have an $\NS$ dependence,
we obtain a picture that suggests very strong violations of diffeomorphism invariance:
Whereas the $\NS$ dependence in $\beta_{G_{(3,0)}}$ comes with $-\,\sqrt{G_{(3,0)}}G_{(1,2)}^{3/2}$,
there is no $\NS$ dependence in $\beta_{G_{(1,2)}}$ (as there is no closed scalar loop that contributes).
...
which clearly feature an opposite sign.
In the case of the background flow, the background-field dependence of the regulator yields extra
...
\end{align}
If we insert this into Eqn.~\eqref{eq:betaG3} and \eqref{eq:betag3},
we obtain the contribution $\sim \NS$ as follows:
\begin{align}
  \beta_{G_{(3,0)}}\Big|_{\NS} &= 3 G_{(3,0)} G_{(1,2)} \frac{\NS}{24 \pi}
  - G_{(3,0)}^{1/2} \,G_{(1,2)}^{3/2}\, \NS\frac{86+\eta_S}{1140 \pi} \nonumber\\
...
Thus, taking into account a non-trivial graviton anomalous dimension leads to a very similar
behaviour for those two different couplings, as suggested by diffeomorphism invariance.
Further, we observe that the same sign of the coefficient as in $\beta_{G_{(1,2)}}$ has been obtained
within the exponential parametrization and the so-called physical gauge, see Eqn.~(59) in
Ref.~\cite{Dona:2015tnf}.

For the background coupling, the coefficient is larger by a factor of 4.
Nevertheless, the three definitions of the coupling share the qualitative behaviour,
and their fixed points grow as a function of $\NS$.
The running of the Newton coupling thus features an effective universality,
\ie different definitions of the coupling lead to qualitative agreement on the running.

The agreement between the fluctuation results and the background result relies on the dominance
of the graviton anomalous dimension in the case of the fluctuation results,
...
Note that the flow of the 3-graviton vertex itself has the opposite sign from the background result,
and the equality of the signs only appears at the level of the couplings.
...
Thus, we stress that it is necessary to evaluate gravitational RG flows at the level of the
fluctuations couplings.
...
\begin{align}\nonumber
	...
	-\frac{\delta S_{\text{\tiny gauge}}[\bar g,h]
}{\delta h_{\mu\nu}}\right\rangle \,,
\label{eq:gaugeNIk}
\end{align}
...
The details of the computation are given in appendix \ref{app:msWItraces}.
...
The first system is the pure background system, the second is the level one improved background system and the last one is the fully fluctuating system.
In the right panel all couplings are bound by the value $\frac 12$, while in the left panel only $\bar \Lambda*(\eta_N,\bar\Lambda^*)$ is bound by that value.}
