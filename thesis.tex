...
\begin{align}
  ...
  \overbar{C}^{\mu} \, M_{\mu\nu} \, C^{\nu} \,,
\end{align}
where $\overbar{C}^{\mu}$ is the conjugate field and the inverse propagator is given by:
\begin{align}
...
In this section we will use the results calculated so far to
obtain the Hessian of pure gravity in the exponential parametrisation
and the resulting graviton propagator in the gauges we have discussed
earlier.
...
using the expressions Eqs.~(\ref{hterms1})--(\ref{hterms3}):
\begin{align}
  \int \mathrm d^dx \, \sqrt{\bar g} \;
  \Bigg[
    \frac{1}{4} \, \hTTmunu
...
propagate unless $\alpha \rightarrow \infty$.
In the de~Donder gauge where $\alpha = 1$ and $\beta=\nicefrac{d}{2}-1$, this expression
becomes:
\begin{align}
  \dots
\end{align}
[propagator for $\hTTmunu$, $\xi^\prime_\mu$, $\sigma^\prime$, $h$, and all ghosts for de~Donder gauge (only) maybe even $d=4$]
...
\begin{align}
  ...
  \Biggl]
\end{align}
[propagator for $\hTTmunu$, $\sigma^\prime$, ghost1 and ghost2 for unimodular gauge]
\bigskip
...
\begin{align}
  ...
  \Bigg] .
\end{align}
[propagator for $\hTTmunu$, $h$, ghost1 for alternative gauge]
