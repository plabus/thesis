...
spin modes. We will again use a symmetric momentum configuration, and
...
\begin{figure}[t]
  ...
  \caption{
    ...
    for the three avatars of Newton's constant:
...
\begin{align}
  ...
  \lambda_3 \, G_{(1,2)}
  \nonumber \\[3pt]
  &+ \frac{64}{171} \, \frac{ (32\pi)^2 \, \sqrt{G_{(3,0)}} }{ k }
  \left(
    \text{Flow}^{(hhh)}_{{\TT},G_{(3,0)}}(k^2)-
    \text{Flow}^{(hhh)}_{{\TT},G_{(3,0)}}(0)
  \right) ,
  \\[5pt]
  \beta_{G_{(1,2)}} &= 2 \, G_{(1,2)} + \eta_h(k^2) \, G_{(1,2)} + 2 \, \etaS(k^2) \, G_{(1,2)}
  \nonumber \\[3pt]
  &+ \frac{ 32 \pi \, \sqrt{G_{(1,2)}} }{ 8 } \, \frac{8}{3} \;
  \text{Flow}^{(h\varphi\varphi)}_{{\TT},G_{(1,2)}}(k^2) \,.
\end{align}
Here, the subscripts ${\TT}, G_{(3,0)}$ and ${\TT}, G_{(1,2)}$
...
\begin{figure}[t]
  ...
  \end{center}
  \caption{
    Numerical results of fixed point values, (real parts of) critical exponents,
...
\begin{align}
  ...
  - \sqrt{G_{(3,0)}} \, G_{(1,2)}^{3/2} \, \NS\, \frac{86}{1140 \pi} \,, \\[3pt]
  \beta_{G_{(1,2)}} &= 2 \, G_{(1,2)} + G_{(1,2)} \, \eta_h \,, \\[3pt]
  \eta_h &= G_{(1,2)} \, \NS \, \frac{1}{24 \pi} \,, \\[3pt]
  \etaS &= 0 \,.
\end{align}
...
\begin{alignat}{5}
  G_{3\, \star}  &= 0 \,,                                  & g_{3\, \star}  &= 0 \,,                         & \theta_{1,2}   &=-2 \,, \\[3pt]
  G_{3\, \star}  &=-\frac{355008 \pi}{9025 \NS} \,, \qquad & g_{3\, \star}  &=-\frac{48 \pi}{\NS} \,, \qquad & \theta_{1,2}   &= 2 \,, \\[3pt]
...
\section{The background field approximation and the msWI}
...
\begin{align}
	\FRG(x)
  = \int_{x_0}^x \mathrm dx' \; \frac{\partial R_k(x')}{\partial x'} \, G_k(x', \overbar R = 0)\,,
  \label{eq:Fg}
\end{align}
...
\begin{align}
  \vphantom{ \frac{\bar \Lambda}{\bar G} }
  \partial_t \bar G
  &= \big( 2 + \etaS \big) \, \bar G \,, \\
  \vphantom{ \frac{\bar \Lambda}{\bar G} }
  \partial_t \bar \Lambda
  &= - 4 \, \bar \Lambda + \frac{\bar \Lambda}{\bar G} \, \partial_t \bar G
  + 8 \pi \bar G \; \Flow_{\bar \Gamma} \Big|_{\mathcal O(\bR^0)} \,, \\
  \etaS &=
   16 \pi \bar G \; \Flow_{\bar \Gamma} \Big|_{\mathcal O(\bR^1)} \,,
\end{align}
where
\begin{align}
...
Note that the quantities in $\Flow_{\bar \Gamma}$ should be taken
...
by setting $\lambda=\bar\Lambda$, $\eta_h=\etaS$ and $\eta_c=0$.


\section{Fluctuation couplings via the Ward identities}
...
\begin{align}
  ...
  \label{eq:def-I}
\end{align}
Here, $\FRG(\bar \Delta)$ is the functional introduced in \eqref{eq:Fg}.
...
coming from the modified split Ward identity.
This yields
\begin{align}
  \vphantom{ \frac{\bar \Lambda}{\bar G} }
  \partial_t \bar G_1
  &= \big( 2 + \etaSfluc \big) \, \bar G_1 \,, \\
  \partial_t \bar \Lambda_1
  &= - 4 \, \bar \Lambda_1 + \frac{\bar \Lambda_1}{\bar G_1} \, \partial_t \bar G_1
  + 8 \pi \bar G_1 \, \big(  \Flow_{\bar \Gamma} - 8 I \big) \Big|_{\mathcal O(\bR^0)} \,, \\
  \vphantom{ \frac{\bar \Lambda}{\bar G} }
  \etaSfluc
  &= 16 \pi \bar G_1 \, \big( \Flow_{\bar \Gamma} - 8 I  \big) \Big|_{\mathcal O(\bR^1)} \,.
\end{align}
...
For this we set $\lambda=\bar\Lambda_1$, $\eta_h=\etaSfluc$ and $\eta_c=0$.
...
\chapter{The msWI in Yang-Mills theory}
\label{app:YM}
