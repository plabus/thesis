...
at the end of the section. The cutoff term we want to add to the classical
...
To conclude this section let us now comment on the case
...
section we will introduce the two schemes which are the most relevant for
...
\end{align}
Similarly, we can express the running of the first three $n$-point
...
\subsection{Fixed points and ultraviolet critical surfaces}
...
which will be responsible for asymptotic safety to
occur or not.

Has we have hinted already above, the easiest way to guarantee that all
couplings of the theory remain finite in the ultraviolet limit
% of the renormalisation group flow,
is to demand that they are all scale-invariant,
\ie do not change their value along the flow,
...
To obtain the dimensionality of the critical surfaces,
...
\begin{align}
  \partial_t \hat g_i =
  \frac{ \partial \overbar \beta_i }{ \partial \overbar g_i}
  \Bigg|_{ \overbar g_k = \overbar g_k^\star }
  \hspace{-7mm}
  \hat g_j
  + \mathcal O (\hat g_i \hat g_j) \,.
\end{align}
Diagonalising the Jacobian
$\nicefrac{ \partial \overbar \beta_i }{ \partial \overbar g_i}$,
it is straightforward to see that the linearised
flow equations for the eigenvectors
$g_i^{\scriptscriptstyle{(d)}}$
assume the form%
\footnote{
  There is no summation over the index $i$ on the left hand side.
}
\begin{align}
  \partial_t \hat g_i^{\scriptscriptstyle{(d)}} =
  \lambda_i \,
  \hat g_i^{\scriptscriptstyle{(d)}}
  + \mathcal O (\hat g_i^{\scriptscriptstyle{(d)}} \hat g_j^{\scriptscriptstyle{(d)}}) \,,
\end{align}
...
the dimensionality of the critical surfaces of the fixed
...
through the behaviour of the quadratic perturbations
...
span the ultraviolet critical surface of the fixed point.
In particular, the number of negative eigenvalues
is equal to the dimensionality of the ultraviolet critical
surface.%
\footnote{
...
Now that we have established how to categorise
a given fixed point as ultraviolet or infrared,
...
\begin{align}
  ...
  + \frac 12 \, \alpha_{ijk} \, \overbar g_j \, \overbar g_k
  + \mathcal O(\overbar g_i\overbar g_j\overbar g_k) \,.
\end{align}
This expansion cannot have a constant term, for
it would spoil $\beta_i(g_j=0) = 0$. Furthermore,
we have $\alpha_{ii} = 0$, because otherwise
the classical scaling would be violated. This
has the immediate consequence that the Jacobian
\begin{align}
  \frac{ \partial \overbar \beta_i }{ \partial \overbar g_i}
...
\end{align}
has eigenvalues given by $-d_i$. But this in turn
implies that the relevant directions, which span
the ultraviolet critical surface, are given
by monomial operators of positive classical mass
dimension, $d_i > 0$. This is nothing but the
property of perturbative power counting renormalisability.
What is more, since for any fixed space-time dimension $d$,
there are only finitely many such (local) operators, the ultraviolet
critical surface cannot be infinite-dimensional.
