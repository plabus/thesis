...
this allows us in principle to solve a quantum field theory by
integrating a well-defined differential equation instead of
calculating the ill-defined path integral. To do this, we fix
any effective average action in theory space a some scale $k_0$
and integrate the flow towards in infrared.

However, the first property mentioned above gives us an other
...
Eqn.~\eqref{expansionGamma} tends to infinity when taking
...
The other possible behaviour of a quantum field theory
is the case where none of the couplings of the theory
...
in the ultraviolet, which was quite suggestively called
\textit{asymptotic freedom}. We will see that
in our language, asymptotic freedom corresponds to a fixed
point of the effective action
under the exact renormalisation group flow,
for which all couplings assume the value zero.
This fixed point is also known as the Gaussian fixed point.
However, there may exist non-trivial (interacting) fixed points
which have non-zero values for (at least some of) the couplings
of the theory, in which case
the theory is said to become \textit{asymptotically save}.
...
emanates from one of these very fixed points and matches
