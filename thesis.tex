...
\newcommand\TT{ \scriptscriptstyle{\mathrm{TT}} }
\newcommand\hTT{ h_{\scriptscriptstyle{\mathrm{TT}}} }
\newcommand\NS{ N_{\scriptscriptstyle{\mathrm{S}}} }
\newcommand\hTTmunu{ h^{\scriptscriptstyle{\mathrm{TT}}}_{\mu\nu} }
\newcommand\etaTT{ \eta_{\scriptscriptstyle{\mathrm{TT}}} }
\newcommand\etaS{ \eta_{\scriptscriptstyle{\mathrm{S}}} }
\newcommand\GNewton{ G_{\scriptscriptstyle{\mathrm{N}}} }
\newcommand{\bF}{\bar F}
\newcommand{\bVstar}{\bar V_\star}
\newcommand{\bFstar}{\bar F_\star}
...
\chapter{Non-minimally coupled Scalar-Tensor theories}

After investigating issues of more technical nature introduced by
the use of the background field method in non-perturbative RG flows
of quantum gravity, we will now start our study of the inclusion of
...
functions $\bar V(\bar \varphi)= k^{-d} V(\varphi)$ and $\bar F(\bar\varphi)=k^{2-d}F(\varphi)$
of the dimensionless field $\bar \varphi=k^{1-d/2}\varphi$
...
\begin{align}
  ...
  \left[ - F(\varphi) \bar R + \frac{1}{2} \, \sum_{i=1}^{\NS} \left( \bar\nabla\varphi^i \right)^2 + V(\varphi)  \right] .
  \label{action}
\end{align}
where now $V$ and $F$ are functions of the radial degree%
\footnote{
...
gauge fixing and resulting Faddeev–Popov ghost sector app. \ref{app:York}.
Further details concerning the derivation of the Hessian and the flow
equations may be found in app. \ref{app:Hessian}.
Finally we use a type-I cutoff, which is introduced by replacing
...
In this section we list some simple analytic solutions which exist in any dimension $d$.
...
Here, it is convenient to use the variable $\bar \rho=\bar \varphi^2/2$
as the argument of the functions $\bV$ and $\bF$ instead of $\bar \varphi$.
It is easy to see that in the large-$\NS$ limit $\bV$, $\bF$ as well
as $\bar \rho$ scale linearly in $\NS$.
In this limit the flow equations---in terms of already rescaled quantities---simply read
\begin{align}
  \dot \bV &= - d \, \bV    + (d-2) \, \bar \rho \, \bV' + \frac{c_d}{1+\bV'} \,, \\
  \dot \bF &= -(d-2) \bF + \left[ (d-2) \, \bar \rho - \frac{c_d}{\left(1+\bV'\right)^2} \right] \bF'  - \frac{d}{12} \, \frac{c_d} {1+\bV'} \,.
  \label{eqlargeN}
\end{align}
...
\begin{align}
  \bVstar &= \frac{c_d}{d} \,, \\
  \bFstar(\bar\rho) &= -\left( \frac{d}{12 \, (d-2)} \, c_d + 2 \, a\right)
  +  2 \, a \, \frac{d-2}{c_d} \, \bar \rho \,,
\end{align}
...
\begin{align}
  \bar \rho = \frac{d \, c_d}{4} \;
  _2F_1 \left( 2,\, 1 - \frac{d}{2};\, 2 - \frac{d}{2};\, -w(\bar \rho) \right) ,
\end{align}
where $d$ is now an odd integer and $w(\bar \rho)=\bV'(\bar \rho)$.
...
\end{align}
where we defined a new function $g(w(\bar \rho)) = \bF (\bar \rho)$.
This equation can be solved analytically and its solution is implicitly given by
\begin{align}
  \bF(\bar \rho) = - \frac{d \, c_d}{12 \, (d-2)} \;
  _2F_1\left( 1,\, 1 - \frac{d}{2};\, 2 - \frac{d}{2};\, -w(\bar \rho) \right) ,
\end{align}
...
We also note that in the asymptotic region
$\bar \rho\to\infty$ one has $\bF(\bar \rho)= -\frac{2 }{3 d (d-2) } \bar \rho$.
...
\begin{align}
  0 &= - (\lambda +4) \, \delta v
       + \left[ \bp - \frac{\NS-1}{32 \pi^2 \bp} \right] \delta v'
       - \frac{1}{32 \pi^2} \, \delta v'' \,, \\
  0 &= - (\lambda +2) \, \delta\! f
       + \left[ \bp - \frac{\NS-1}{32\pi^2\bp} \right] \delta\! f'
       + \frac{\NS-1}{96\pi^2\bp} \, \delta v'
...
\begin{align}
  ...
  \theta_5 &= 0 \,, \qquad w_5^t = (\delta u, \, \delta\! f)_5 =                     \left( 0,\, -\frac{\NS}{32\pi^2}+\bp^2 \right) .
\end{align}
...
\begin{align}
  0 &= - (\lambda + 4) \, \delta v
       + \frac{72 \, \lambda}{169 - 12 \NS} \, \delta \!f
       + \left[ \bp - \frac{\NS-1}{32 \pi^2 \bp} \right] \delta v'
       + \frac{72 \, \bp}{12\NS - 169} \, \delta\!f' - \frac{1}{32\pi^2} \, \delta v'' \,, \\
  0 &=   \left[ \frac{3 \, \lambda \, (45-4\NS)}{12\NS - 169}-2 \right] \delta\!f
       + \left[ \frac{3 \, (4\NS - 45) \, \bp}{12\NS-169}
       - \frac{\NS-1}{32\pi^2\bp} \right] \delta\!f'
       - \frac{1}{32\pi^2} \, \delta\!f'' \nonumber \\
       & \hspace{9.65cm}
         + \frac{\NS-1}{96\pi^2\bp} \, \delta v'
         + \frac{1}{96\pi^2} \, \delta v'' \,.
...
\begin{alignat}{2}
  ...
  \theta_2 &= 2 + \frac{68}{ 3 (45 - 4\NS) } \,, \quad  && (\delta v, \, \delta\! f)_2 = \left( \frac{72}{12 \NS - 101}, \, 1 \right)  , \nonumber\\
  \theta_3 &= 2 \,,                                   && (\delta v, \, \delta\! f)_3 = \left( -\frac{29 \NS}{544\pi^2} + \bp^2, \, \frac{\NS(169 - 12 \NS)}{3264\pi^2}\right) , \nonumber\\
  \theta_4 &= \frac{68}{ 3 (45 - 4\NS) } \,,            && (\delta v, \, \delta\! f)_4 = \bigg( -\frac{3\NS(48\NS(3 \NS - 76)+22475)}{16\pi^2(4\NS - 45)(6\NS - 59) (12\NS - 101)} \nonumber \\
                                                    & && \hspace{1.1cm} + \frac{72}{12\NS - 101} \, \bp^2, \,  - \frac{\NS(12\NS - 169)(12\NS-125)} {96\pi^2(4\NS - 45)(12 \NS - 101)}+\bp^2\bigg) , \nonumber \\
  \theta_5 &= 0 \,,                                   && (\delta v, \, \delta\! f)_5 = \bigg( \frac{29 \NS (\NS+2)}{17408\pi^4} - \frac{29(\NS+2)}{272\pi ^2} \, \bp^2 + \bp^4, \nonumber \\
...
In ref. \cite{Percacci:2015wwa} we looked for scaling solutions in $d=3$
with a potential resembling that of the Wilson-Fisher fixed point.
We concentrated on the unimproved equations,
and found a solution for sufficiently small $\bp$
whose potential is almost indistinguishable from the
Wilson-Fisher potential, and with a function $\bF$
that starts out positive at $\bp=0$ but has negative
...
a global solution was found for the improved RG equation ref. \cite{Borchardt:2015rxa}.
...
simpler techniques used in ref. \cite{Percacci:2015wwa}, such as
...
\end{align}
The shape of the potential $\bV$ shows that the scaling solution is characterised by a broken phase.
...
\end{align}
For the case $\NS=2$ there is a very good match between the numerical solution
found with the shooting method around the origin
and the asymptotic behaviour given above for $A\simeq 5.149$ and $B\simeq 0.273$ around $\bp\simeq0.65$.
...
The critical exponents associated to this fixed point ($\NS=2$) have not been determined with
sufficient precision using polynomial methods.
We can only report that they are not too far from the values obtained for the $\NS=1$ case.
...
As discussed in ref. \cite{Bridle:2013sra}, already in pure scalar theory
...
have to be dealt with by other means, see e.g. refs. \cite{Dietz:2015owa,Becker:2014qya}.
...
a cutoff $\gamma \, k^{d-2} \, (k^2-z) \, \theta(k^2-z)$,
...
\begin{align}
  \dot \bV &= - 4 \bV + \bp \, \bV' - \frac{1}{8\pi^2}
            ...
            { 144 \pi^2 \bF'{}^4 } \nonumber \\[3mm]
  &         + \frac{ 3 \gamma \left[ - \gamma^2 + 4\gamma \bF - 2\gamma(\gamma-2\bF) \log \left( \frac{\bF}{\gamma} \right) - 3 \bF^2\right]}
                   { 16 \pi ^2 (\gamma -\bF)^3}+\frac{(\NS-1) \bp }{32 \pi ^2 \left(\bV'+\bp \right)} \,, \\[5mm]
  ...
  &         + \frac{ 10 \gamma  (\gamma -2 \bF) (\gamma -\bF)-5 \gamma  \left(\gamma ^2-3 \gamma  \bF+4 \bF^2\right) \log \left(\frac{\bF}{\gamma }\right)}
          { 96 \pi ^2 (\gamma -\bF)^3} \nonumber \\[3mm]
  &         + \frac{ \gamma\left[-\gamma +(\gamma-2\bF)\log\left(\frac{\bF}{\gamma}\right)+\bF\right]}
                   {96\pi^2(\gamma-\bF)^2}
            - \frac{(\NS-1) \bp  \left(3 \bF'+\bV'(\bp )+\bp \right)}{96 \pi ^2 \left(\bV'+\bp\right)^2} \,.
\end{align}
Unlike eqns. (\ref{flowvfull}) and (\ref{flowffull}),
these are transcendental equations and cannot be solved analytically.
For a comparison we discuss only the  scaling solution FP1
which is defined by $\bV(\bp)=\bV_0$ and $\bF(\bp)=\bF_0$.
With this ansatz the fixed point conditions reduce to
the following algebraic equations
\begin{align}
  \label{eqpureFP1}
  \bV_0 &= \frac{1}{128\pi^2(\gamma-\bF_0)^2} \bigg[ (\NS-10)\gamma^2-2(\NS-13)\gamma \bF_0+(\NS-4)\bF_0^2  \nonumber \\
        &\hspace{8cm} -12 \, \gamma^2 \, \frac{ \gamma - 2\bF_0 }{ \gamma - \bF_0} \, \log \left( \frac{\bF_0}{\gamma} \right) \bigg] \\
  0   &= -2\bF_0-\frac{4N-55}{384\pi^2} -\frac{\bF_0(\gamma+9\bF_0)}{96\pi^2(\gamma-\bF_0)^2} -\frac{\gamma\left(2\gamma^2-6\gamma \bF_0+9\bF_0^2\right) \log\left(\frac{\bF_0}{\gamma}\right)}{48\pi^2(\gamma-\bF_0)^3}
\end{align}
These can be solved numerically.
The linearisation of the flow equation around FP1 yields the following
equations:
\begin{align}
  0 &= - (\lambda+4) \, \delta v
...
For $\NS=1$ and $\gamma=1$ we have $\bV_{0 \, \star}=0.03314$, $\bF_{0 \, \star}=0.01552$.
...
For $\gamma=0.006$ we have $\bV_{0\,\star}=0.00353$, $\bF_{0\,\star}=0.00670$,
...
which are also closer the other cutoff.

For $\NS=4$ and $\gamma=1$ the fixed point is at $\bV_{0\,\star}=0.03635$, $\bF_{0\,\star}=0.01413$.
The linear perturbation analysis around the fixed point gives the following eigenvalues: $-4$, $-2.299$, $-2$, $-0.299$, $0$.
If instead we choose $\gamma=0.006$, we have $\bV_{0\,\star}=0.00669$, $\bF_{0\,\star}=0.00549$,
...
eqns.~\eqref{eqpureFP1} and \eqref{other_eqn} for FP1 in the large-$\NS$ limit.
It is easy to see that in this limit they become
\begin{align}
  \bV_{0 \, \star} \approx \frac{ 16 \NS - 205 }{ 512 \pi^2 } \quad , \quad
  \bF_{0 \, \star} \approx \gamma\exp{\frac{ 55 - 4 \NS }{16}}\ .
\end{align}
