...
\begin{align}
  \text{G:} \quad &\big\lbrace 2 ,\, 4 - d \big\rbrace \,, \\
  \text{WF:} \quad &\left\lbrace
  - \frac 16 \left( 10 - 4d \pm \sqrt{ 484 - 200 d + 22 d^2 } \right)
    \right\rbrace .
\end{align}
...
we can choose the set the field configuration to a constant,
which truly makes the potential a function of the parameter $\varphi$,
...
\begin{align}
  ...
  \right] .
\end{align}
Note that the value of the $Q$-functional depends on the choice of
the cutoff. In general it is good practise to test results for the values
...
Here, however, we will only use the optimised cutoff, since it allows
...
% Derivation of dimensionless flow equation,
% Polynomial Approximations w/ optimised cutoff and Wilson-Fisher fixed point,
% Scaling solutions, spike plots
