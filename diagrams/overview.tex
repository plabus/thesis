\begin{tikzpicture}[
  ...
  % Top rule
  \draw[line width=1.5pt] (-4.5,  2.00) -- (4.5,  2.00);
  % Mid rule
  \draw[line width=0.55pt](-4.5,  1.25) -- (4.5,  1.25);
  % Bottom rule
  \draw[line width=1.5pt] (-4.5, -2.00) -- (4.5, -2.00);

  % Main topics on the right
  \node                 at (-7.5,  1.63) {main topic};
  \node[rectangle,draw] at (-7.5,  0.0)  {background independence};
  \node[rectangle,draw] at (-7.5, -1.0)  {scalar matter + gravity};
  ...
  \draw[->] (table-4-1.west) -- (-5.5, -0.9);
  \draw[->] (table-5-1.west) -- (-5.5, -1.0);
  \draw[->] (table-6-1.west) -- (-5.5, -1.1);
