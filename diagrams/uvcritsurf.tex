...
\begin{tikzpicture}[
  ...
  \tikzmath{
    ...
    \degs      = 35;
    \degsinv   = 180-\degs;
    \dotradius = 0.03; % radius of the circles that make the dots
    \basex     = 6.5;
    \basey     =-0.5;
  }
  ...
  \draw[thick,domain=\degs:\degsinv] plot ({\radius*cos(\x)+\basex}, {\radius*sin(\x)+\basey});
  \draw[thick,] ($ (base) + ({ cos(\degs)*\radius}, {sin(\degs)*\radius}) $) arc (90:128.2:\radiusa);
  \draw[thick,] ($ (base) + ({-cos(\degs)*\radius}, {sin(\degs)*\radius}) $) arc (90: 41.5:\radiusb);
  ...
  \draw[->] (FP) -- ($ (FP) + (dirx) $);
  \node at ($ (FP) + ( 0.38,-0.52) $) {$\scriptscriptstyle{\hat g}_{\scaleto{1}{2pt}}$};
  \draw[->] (FP) -- ($ (FP) + (diry) $);
  \node at ($ (FP) + (-0.55,-0.46) $) {$\scriptscriptstyle{\hat g}_{\scaleto{2}{2pt}}$};
  \draw[->] (FP) -- ($ (FP) + (dirz) $);
  \node at ($ (FP) + (-0.15, 0.56) $) {$\scriptscriptstyle{\hat g}_{\scaleto{3}{2pt}}$};
  ...
  % IR critical surface
  \draw[midarrow,        line width=0.01mm] ($ (FP) + (IR) $) -- (FP);
  \draw[midarrow-dashed, line width=0.01mm] ($ (FP) - (IR) $) -- (FP);
  \draw[midarrow-rev,    line width=0.01mm] ($ (FP) - (IR) $) -- ($ (FP) - 1.5*(IR)$);

  % fundamental theory, with trajectory and dot
  \draw[fill] (Gamma) circle [radius=\dotradius] node[left] {$\Gamma$};
  \draw[midarrow] (FP) arc (90:114:1.4*\radius);
  ...
  node[above] {$\Gamma'$};
  \draw[midarrow-dashed] (A)  .. controls (controlAB) .. (B);
